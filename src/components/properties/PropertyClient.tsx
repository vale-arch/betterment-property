...
export default function PropertyClient({ property }: { property: any }) {
  ...
  // CLEAN WHATSAPP MESSAGE (No weird IDs)
  const message = `Hi, I'm interested in viewing *${property.title}* in ${property.sub_counties?.name || 'this area'}. Is it still available?`;
  ...
  return (
    <div style={{ background: '#FDFCF9', minHeight: '100vh', color: '#1B1464', paddingBottom: '120px' }}>
      <style>{`
        ...
        .mobile-actions { position: fixed; bottom: 0; left: 0; right: 0; background: rgba(255,255,255,0.98); backdrop-filter: blur(10px); padding: 15px 20px; display: none; gap: 10px; border-top: 1px solid #EEE; z-index: 9999; }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '30px' }}>
          <div>
            <div className="tooltip-container">
              <span style={{ background: '#F5EFFF', color: '#7B2CBF', padding: '4px 12px', borderRadius: '6px', fontSize: '10px', fontWeight: '900', letterSpacing: '1px' }}>VERIFIED ASSET ✓</span>
            ...
            <h1 style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 800, margin: '5px 0', lineHeight: 1, letterSpacing: '-1.5px', color: '#2D004F' }}>{property.title}</h1>
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <div style={{ color: '#2D004F', fontSize: '3.5rem', fontWeight: '800', fontFamily: 'Bebas Neue', lineHeight: 1 }}>KES {property.price?.toLocaleString()}</div>
          </div>
        </div>
      </div>
      ...
      <div className="main-wrapper">
        ...
        <div className="desktop-sidebar">
           <div className="sticky-sidebar">
              ...
              {sent ? (
                <div style={{ background: '#F0FDF4', color: '#166534', padding: '20px', borderRadius: '12px', textAlign: 'center', fontSize: '14px', border: '1px solid #BBF7D0' }}><b>SUCCESS</b><br/>The agent has been notified.</div>
              ) : (
      ...
      <div className="mobile-actions">
          <a href={`tel:${property.profiles?.phone}`} style={{ flex: 1, textDecoration: 'none' }}>
            <button style={{ width: '100%', background: '#FFF', color: '#2D004F', border: '1px solid #DDD', padding: '18px', borderRadius: '12px', fontSize: '12px', fontWeight: '800' }}>CALL AGENT</button>
          </a>
